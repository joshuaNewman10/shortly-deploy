...
var client = new mongodb.Db('shortly', server);
...
client.open(function(err, p_client) {
  ...
  client.createCollection("links", function(err, collection) {
    console.log("Created collection");

    // Here's the document we want to insert:
    var document = {url: "http://google.com",
                    title: "Google",
                    base_url: "http://googe.com"};
    ...
    collection.insert(document, function(err, docs) {
      ...
      collection.find().toArray(function(err, results) {
        ...
        for (var i = 0; i < results.length; i++) {
          console.log(results[i].url, results[i].title, results[i].base_url);
        }
